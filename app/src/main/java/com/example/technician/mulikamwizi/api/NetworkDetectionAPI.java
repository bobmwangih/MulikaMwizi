...
import com.example.technician.mulikamwizi.model.NetworkPojo;
...
public class NetworkDetectionAPI {
    ...
    public static boolean isNetworkChanged(Context context) {
        Set<String> networkIds = new HashSet<>(Arrays.asList("networkId", "networkName", "networkState", "isWifiNetwork"));
        ...
        if (availableNetwork != null) {
            ...
            if (data.keySet().size() > 0) {
                if (availableNetwork.getWifiNetwork().equals("true") && Objects.equals(data.get("isWifiNetwork"), "false")) {
                    notifyUser = true;
                }
